...
class ListUserClientController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $total = auth()->user()->can('manage-all-users') ? User::where('user_group_id', 5)->count() : User::where([['user_group_id', 5], ['parent_id', auth()->user()->id]])->count();
        $new_users = auth()->user()->can('manage-all-users')  ? User::where([['user_group_id', 5], ['created_at', '>=', Carbon::now()->startOfWeek()], ['created_at', '<=', Carbon::now()->endOfWeek()]])->count() : User::where([['user_group_id', 5], ['parent_id', auth()->user()->id], ['created_at', '>=', Carbon::now()->startOfWeek()], ['created_at', '<=', Carbon::now()->endOfWeek()]])->count();

        $site_options['site_name'] = $db_settings->settings['site_name'];
        $site_options['sub_name'] = $language[12];
